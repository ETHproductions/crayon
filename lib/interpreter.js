    ...
    run: function run(code) {
        ...
        for (i = 0, level = 0; i < tokens.length; i++) {
            ...
            if (nexts.length && i === nexts.last[2]) {
                ...
                state.stack.unshift(t[0] === "`" ? t[1] : t[0] === "'" ? eval("/(?:)/") : t[0] === '"' ? eval(t.replace(/`(.)/g, "\\$&")) : Big(t));
            } else if (controls.test(t)) {
                nexts.push(map[i]);
                if (t === "O") {
                    ;
                } else if (t === "!") {
                    currs.push(!state.stack[0] || state.stack[0].length === 0);
                } else if (t === "z") {
                    currs.push(!!state.stack[0] && state.stack[0].length !== 0);
                } else if (t === "=") {
                    currs.push(2 in state.stack && state.stack[0].toString() === state.stack[1].toString() && ty(state.stack[0]) === ty(state.stack[1]));
                }
            } else if (t === "{") {
                i = nexts.pop()[0];
            } else if (t === "}") {
